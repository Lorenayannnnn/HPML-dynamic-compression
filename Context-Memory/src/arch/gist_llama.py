"""
Huggingface LLaMA with comp (gist) token support.
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        key_states = self.k_proj(hidden_states).view(bsz, q_len, self.num_key_value_heads,
                                                     self.head_dim).transpose(1, 2)
        value_states = self.v_proj(hidden_states).view(bsz, q_len, self.num_key_value_heads,
                                                       self.head_dim).transpose(1, 2)
        ...
        # Note: In transformers 4.40+, rotary_emb.forward() signature changed from
        # (x, seq_len=...) to (x, position_ids). We use position_ids directly.
...
class GistLlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        comp_mask: Optional[torch.Tensor] = None,
        comp_token: Optional[torch.LongTensor] = None,
        sum_mask: Optional[torch.Tensor] = None,
        sum_attn_mask: Optional[torch.Tensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        Args:
            ...
            return_dict (`bool`, *optional*):
                ...
                plain tuple.
        """
        # Get compression masks for conditional inference and memory update
